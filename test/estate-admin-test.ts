  ...
  it("Creates an estate admin", done => {
    ...
    chai
      ...
      .then((res: any) => {
        ...
        let object = delete (res.body.data).password;
        let admin = delete newAdmin.password;
        chai.expect(object).to.deep.include(admin); // expression which will be true if the document created contains the body that was sent
        done();
      })
      .catch(done);
  });
  it("Updates an estate admin", done => {
    ...
    chai
      ...
      .then((res: any) => {
        ...
        chai.assert.exists(res.body.data.id); // assertion expression which will be true if id exists
        //chai.expect(res.body.data).to.deep.include(delete newAdmin.password); // expression which will be true if the document created contains the body that was sent
        done();
      })
      .catch(done);
  });
